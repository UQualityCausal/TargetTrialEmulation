...
?? add a DAG to illustrate the underlying relationship between variables???
```{r}
...
table(data$Event)
```
Among the 729 subjects with no events, 90 were early dropouts and 639 were administratively censored.
```{r}
...
# 
# # Calculate RMSE
# calculate_rmse <- function(estimated_vector, true_vector) {
#   sqrt(mean((estimated_vector - true_vector)^2))
# }
# 
# # Package column
# package_rmse <- c(
#   "—",  
#   sprintf("%.3f", calculate_rmse(results_1_package$ATE, results_true$ATE)),
#   sprintf("%.3f", calculate_rmse(results_2_package$ATE, results_true$ATE)),
#   sprintf("%.3f", calculate_rmse(results_3_package$ATE, results_true$ATE)),
#   sprintf("%.3f", calculate_rmse(results_4_package$ATE, results_true$ATE))
# )
# 
# # Manual column
# manual_rmse <- c(
#   sprintf("%.3f", calculate_rmse(results_crude$ATE, results_true$ATE)),
#   sprintf("%.3f", calculate_rmse(results_1_manual$ATE, results_true$ATE)),
#   sprintf("%.3f", calculate_rmse(results_2_manual$ATE, results_true$ATE)),
#   sprintf("%.3f", calculate_rmse(results_3_manual$ATE, results_true$ATE)),
#   sprintf("%.3f", calculate_rmse(results_4_manual$ATE, results_true$ATE))
# )
# 
# rmse_table <- data.frame(
#   Scenario = scenarios, Package = package_rmse, Manual = manual_rmse)
# 
# kable(rmse_table, col.names = c("Scenario", "Package RMSE", "Manual RMSE"),
#       caption = "Performance of the DR estimator", align = c('l', 'c', 'c')) %>%
#   kable_styling(full_width = FALSE, font_size = 12) %>%
