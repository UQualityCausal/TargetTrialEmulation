...
Assuming loss to follow-up is completely at random, i.e., $C \perp T^{(d)}$, @DR-pseudo proposed a DR estimator using pseduo observations.
...
generate_data_noninf <- function(n = 1000) {
  ...
  logit_p <- (0.05 * Z1 + 3.5 * Z2 + 2.5 * Z3)/6
  p <- exp(logit_p) / (1 + exp(logit_p))
  D <- rbinom(n, 1, p) # Treatment indicator
  
  hazard <- exp(-0.5 + 1 * D + 0.6 * Z1 + 0.8 * Z2 + 0.7 * Z1 * Z2)

  T_event <- rexp(n, rate = hazard) # Event time from an exponential model
  T_censor <- rexp(n, rate = exp(-3.2)) # Censoring time based on random censoring
...
get_true_survival <- function(data, t, d_level) {
  hazard <- exp(-0.5 + 1 * d_level + 0.6 * data$Z1 + 0.8 * data$Z2 + 0.7 * data$Z1 * data$Z2)
...
```{r}
n_data <- 100
datasets <- vector("list", n_data)
for (i in 1:n_data) {
  datasets[[i]] <- generate_data_noninf()
}

times <- c(1, 2, 3)
true_ate_samples <- matrix(NA, nrow = n_data, ncol = length(times))
...
kable(results_manual_tbl, caption = "ATE (95% CI)", align = c('l', 'c', 'c', 'c')) %>% 
...
calculate_rmse <- function(datasets, true_ate_samples, times, method = "package", 
                                     ps_correct = TRUE, or_correct = TRUE) {
  
  n_data <- length(datasets)
  
  results <- future_lapply(1:n_data, function(i) {
    require(survival)
    require(pseudo)
    ...
    est_results <- switch(
      method,
      "package" = pseudo_dr_ate_pkg(datasets[[i]], times, 
          ps_correct = ps_correct, or_correct = or_correct, use_bootstrap = FALSE),
...
```{r}
rmse_1_package <- calculate_rmse(datasets, true_ate_samples, times, method = "package",
                                  ps_correct = TRUE, or_correct = TRUE)
rmse_2_package <- calculate_rmse(datasets, true_ate_samples, times, method = "package",
                                  ps_correct = TRUE, or_correct = FALSE)
rmse_3_package <- calculate_rmse(datasets, true_ate_samples, times, method = "package",
                                  ps_correct = FALSE, or_correct = TRUE)
rmse_4_package <- calculate_rmse(datasets, true_ate_samples, times, method = "package",
                                  ps_correct = FALSE, or_correct = FALSE)

rmse_1_manual <- calculate_rmse(datasets, true_ate_samples, times, method = "manual",
                                 ps_correct = TRUE, or_correct = TRUE)
rmse_2_manual <- calculate_rmse(datasets, true_ate_samples, times, method = "manual",
                                 ps_correct = TRUE, or_correct = FALSE)
rmse_3_manual <- calculate_rmse(datasets, true_ate_samples, times, method = "manual",
                                 ps_correct = FALSE, or_correct = TRUE)
rmse_4_manual <- calculate_rmse(datasets, true_ate_samples, times, method = "manual",
                                 ps_correct = FALSE, or_correct = FALSE)
...
# Package column
package_rmse <- c("—", format_rmse(rmse_1_package), format_rmse(rmse_2_package), 
                  format_rmse(rmse_3_package), format_rmse(rmse_4_package))

# Manual column
manual_rmse <- c(format_rmse(rmse_crude), format_rmse(rmse_1_manual), format_rmse(rmse_2_manual), 
                 format_rmse(rmse_3_manual), format_rmse(rmse_4_manual))

rmse_table <- data.frame(Scenario = scenarios, Package = package_rmse, Manual = manual_rmse)
