...
generate_data_noninf <- function(n = 2000) {
  ...
  logit_p <- (-0.1 + 0.15 * Z1 + 1.2 * Z2 + 5 * (Z1 * Z2) + 2 * (Z2^2)) / 8
...
pseudo_dr_ate_pkg <- function(data, times, ps_correct = TRUE, or_correct = TRUE, use_bootstrap = TRUE) {
  ...
  if (ps_correct) {
    ...
    ps_model <- glm(D ~ Z4 + I(Z4^2), data = data, family = "binomial")
  }
...
pseudo_dr_ate_manual <- function(data, times, ps_correct = TRUE, or_correct = TRUE) {
  ...
  if (or_correct) {
    ...
    or_model_1 <- coxph(Surv(Time, Event) ~ Z4 + Z3, data = data_1)
    or_model_0 <- coxph(Surv(Time, Event) ~ Z4 + Z3, data = data_0)
  }
  ...
  if (or_correct) {
    ...
    base_surv_1 <- survfit(or_model_1, newdata = data.frame(Z4 = 0, Z3 = 0))
    base_surv_0 <- survfit(or_model_0, newdata = data.frame(Z4 = 0, Z3 = 0))
  }
