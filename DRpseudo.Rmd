...
generate_data_noninf <- function(n = 1000) {
  ...
  logit_p <- (0.5 * Z1 + 3.5 * Z2 + 2.5 * Z3)/6
  ...
  T_censor <- rexp(n, rate = exp(-5.0)) # Censoring time based on random censoring
...
calculate_rmse <- function(datasets, true_ate_samples, times, method = "package", ps_correct = TRUE, or_correct = TRUE) {
  ...
  results <- future_lapply(1:n_datasets, function(i) {
    ...
    est_results <- switch(
      method,
      "package" = pseudo_dr_ate_pkg(datasets[[i]], times, ps_correct = ps_correct, 
                                    or_correct = or_correct, use_bootstrap = FALSE),
