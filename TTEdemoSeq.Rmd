...
author: "Kevin Ying, Peirong Hao, Adam Bress, Tom Greene, Yizhe Xu*"
date: "`r Sys.Date()`"
...
-   **Step 2: Clarify study design**
    ...
    -   Individuals in the control group who meet these eligibility criteria in Table 1 continuously would be eligible for the target trial at multiple times during their lifetime, that is, they have multiple times that can qualify as time zero. When should their follow-up start in the observational study?
    -   Two unbiased options:
        -   **All eligible times**:
            ...
            -   More complicated analysis procedures to account for using the same individual's data multiple times in the study
...
-   **Step 3: Build a data set that fits the purpose of the study**
    ...
    -   To build a fit-for-purpose dataset, @Danaei2013 demonstrated how to emulate a sequence of trials with observational data using the *TrialEmulation* R package
...
In the TrialEmulation paper notation, this emulated trial indicator is the trial index $m$. In this tutorial, that variable is `trial_period` after data preparation.

When interpreting marginal treatment effects from a sequence of trials, it is important to state the target trial population. Here we target the pooled population represented by eligible participants across the selected trial periods.

Before starting using the *TrialEmulation* R package, it is crucial to understand the process being carried out in the *data_preparation* function. We explain the steps using a toy example where we only consider three enrollment times: at the beginning of the overall study, month 1, and month 2.
...
obsdata2 <- obsdata1 %>%
  ...
  mutate(eligible = as.integer(age >= 50 & cumsum(Y) == 0 & (slide_dbl(
    ...
    .complete = T
  ) == 0))) %>% # Age over 50, no history of CVD, and no ACEI or ARB treatment in last 24 months.
...
```{r}
# Find the first date when some individuals become eligible. This is the start date that we can start enroll subjects
...
```{r}
paper_map_seq <- tibble::tribble(
  ~Paper_component, ~Code_in_this_tutorial,
  "Sequential trial index m", "trial_period",
  "Expanded pooled trial data", "prep_ITT_data$data",
  "Target trial population", "Eligible participants pooled over first_period to last_period"
)

knitr::kable(paper_map_seq, caption = "Paper-to-Code Mapping (Sequential Trials)", row.names = FALSE)
```
...
It shows that all the variables are the same between the two data sets though finer checking can be made. Both datasets are ready to be used for downstream analyses now.
