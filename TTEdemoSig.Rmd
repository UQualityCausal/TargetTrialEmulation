...
- Subject 21 is not eligible from time 1999-06-01 to 2001-11-01 due to age below 50. Subject 21 becomes eligible at time 2001-12-01 when he turns 50 and he did not take ACEI or ARB treatment in the last 24 months.
```{r}
obsdata1 %>% filter(id==21)
```
...
Let’s verify one example. Subject 21 first becomes eligible at time 2000-07-0,and he initials treatment at time 2003-05-01. So ($T_0$) is 2003-05-01 for him.
```{r}
time0 %>% filter(id == 21)
...
obsdata3 <- obsdata2 %>%
  filter(time >= t0)   # include only eligible subjects who initials treatment
...
- ?? It is a good practice to prespecify a **index window**, say 01/01/2001 - 01/01/2004, and a study period say 01/01/2017 - 01/01/2024, so that individuals are followed up for a sufficient amount of time, i.e., 4 years
```{r}
obsdata4 <- obsdata3 %>% filter(t0>= as.Date("2001-01-01") & time < as.Date("2004-01-01")) %>%
