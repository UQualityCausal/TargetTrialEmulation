...
- Observational data is best used to emulate **a pragmatic trial** in the sense that 
  ...
  - Treatments are compared under the usual conditions used 
...
- Benefits of using active control 
  ...
  - In contrast, in the example of hormone therapy versus no treatment, patients who use hormone therapy can be very different than those who do not use the therapy in terms of socioeconomic status and body mass index

## Assignment Procedures
- It is infeasible to emulate trials with blind assignment as individuals are aware of the treatment they received in observational data
...
- To achieve conditional exchangeability, we need to adjust for all confounding covariates using causal inference methods (see [@Hernan2020] for more details)
...
- **Step 3: Build a data set that fits the purpose of the study**
  ...
  - It is a false belief that one data set can be used to answer any questions. 
...
**Component 1: Time 0: the time point a subject satisfies the eligibility criteria and initiates treatment** 
...
- We apply the rest of eligibility criteria (the first two) in Table 1 to our data and create an "eligible" indicator
```{r}
obsdata1 <- obsdata %>%
  group_by(id) %>%
  mutate(eligible = as.integer(age >= 50 & cumsum(Y) == 0 & (slide_dbl(
    ...
    .complete = T
  ) >= 24))) %>% # Age over 50, no history of CVD, and no ACEI or ARB treatment in last 24 months.
  mutate(eligible = ifelse(is.na(eligible), 0, eligible)) %>% # If unknown, then not eligible
  ungroup()
```

Let us see when Subject 24 becomes eligible: 
- Subject 24 is not eligible from time 2001-03-01 to 2002-06-01 due to age below 50. He is still not eligible until 2003-03-01 when he has enough records to show that he did not receive ACEI or ARB treatment in the last 24 months.
```{r}
obsdata1 %>% filter(id == 24)
```

Next, we identify time 0 ($T_0$) at which a subject becomes eligible and initiates a treatment.
```{r}
time0 <- obsdata1 %>% filter(eligible == 1 & !is.na(A)) %>%
  group_by(id) %>%
  summarise(
    t0 = min(time)) %>%
  ungroup()
...
Check the number of subjects enrolled in analysis.
```{r}
nrow(time0)
```

Let’s verify one example. Subject 24 first becomes eligible at time 2003-03-01, and he initiates treatment at time 2004-05-01. So ($T_0$) is 2004-05-01 for him.
```{r}
time0 %>% filter(id == 24)
...
```{r}
obsdata2 <- obsdata1 %>%
  left_join(time0, by = "id") # Merge the eligibility information to our observational data
attr(obsdata2$eligible, "label") <- "Eligibility indicator at each time in records"
attr(obsdata2$t0, "label") <- "Start of follow-up"

obsdata3 <- obsdata2 %>%
  filter(time >= (t0 - 31)) # Include only eligible subjects who initiates treatment and previous month to get the baseline

# Check every subject has exactly one month of data before time 0
obsdata3 %>% filter(time < t0) %>% group_by(id) %>% summarise(n = n()) %>% filter(n != 1) # expect 0 rows
```
...
- Individuals who initiated both ARB and ACEI medications are excluded 

**Component 3: Treatment assignment**

- Although ARBs and ACEIs are both first-line treatment and used interchangeably at clinic, they are not completely randomized to patients in our observational data, which may lead to imbalance in patient characteristics between treatment groups
...
- Formally, we assume treatment assignment is independent of the CVD outcome conditional on all the baseline covariates mentioned above, i.e., conditional exchangeability assumption
...
- The conditional exchangeability assumption becomes more plausible as more confounders are measured and considered though we can never verify the assumption completely holds true
- We will demonstrate how to restore the conditional exchangeability using causal inference methods in the tutorial of [Intention-to-treat Analysis](https://uqualitycausal.github.io/TTECausalR/IntentionToTreat.html).
...
- We accrue subjects in period 01/01/2001 - 01/01/2004, and stop collecting data at 01/01/2008, so the maximum follow-up time is 84 months.
```{r}
obsdata4 <- obsdata3 %>% filter(t0 >= as.Date("2001-01-01") & t0 < as.Date("2004-01-01") & time < as.Date("2008-01-01")) %>%
  mutate(follow_up = as.integer(time - t0)) %>%
  select(-t0, -eligible)
```

Finally, we save the data for downstream statistical analysis 
```{r}
# Extract baseline data: last follow-up < 0 for each id
baseline <- obsdata4 %>%
  filter(follow_up < 0) %>%
  group_by(id) %>%
  slice_max(order_by = follow_up, n = 1, with_ties = FALSE) %>%  # Closest to 0 from below
  ungroup() %>%
  select(id, X1, X2, age) %>%
  rename(
    X1_0  = X1,
    X2_0  = X2,
    age_0 = age
  ) # The baseline values are the values of the month before treatment initiation.

assigned.trt <- obsdata4 %>%
  filter(follow_up == 0) %>%
  select(id, A) %>%
  rename(assigned_treatment = A) # The assigned treatment is the treatment at initiation.

baseline <- baseline %>%
  left_join(assigned.trt, by = "id") %>%
  select(id, assigned_treatment, everything())
...
attr(obsdata5$follow_up, "label") <- "Follow-up time"
