...
**Component 1: Time 0: the time point a subject satisfies the eligibility criteria and initial treatment** 
...
Let us see when Subject 15 becomes eligible: 
- Subject 15 is not eligible from time 1997-11-01 to 2000-09-01 due to age below 50. Subject 21 becomes eligible at time 2000-10-01 when he turns 50 and he did not take ACEI or ARB treatment in the last 24 months.
```{r}
obsdata1 %>% filter(id==15)
```

Next, we identify time 0 ($T_0$) at which a subject becomes eligible and initials a treatment.
```{r}
...
Let’s verify one example. Subject 15 first becomes eligible at time 2000-07-0,and he initials treatment at time 2002-10-01. So ($T_0$) is 2002-10-01 for him.
```{r}
time0 %>% filter(id == 15)
...
obsdata3 <- obsdata2 %>%
  filter(time >= (t0-31))   # include only eligible subjects who initials treatment and previous month to get the baseline
...
- We will demonstrate how to restore the conditional changeability using causal inference methods in the tutorial of [Intetion-to-treat Analysis](https://uqualitycausal.github.io/TTECausalR/IntentionToTreat.html).
...
- We acrual subjects in period 01/01/2001 - 01/01/2004,and stop collect data at 01/01/2008, so the maximum follow-up time is 84 months.
```{r}
