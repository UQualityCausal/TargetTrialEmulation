...
coordinates(dag) <- list(
  x = c(Z1 = 0, Z2 = 0.5, Z3 = 0, Z4 = 0.5, Treatment = 0.5, Death = 1.5),
  y = c(Z1 = 1, Z2 = 0.55, Z3 = -0.2, Z4 = 0.15, Treatment = 0.35, Death = 0.35)
)
plot(dag)
```
```{r fig.width=3, fig.height=2}
...
coordinates(dag) <- list(
  x = c(Z1 = -0.2, Z2 = -0.2, Z3 = -0.2, Z4 = -0.2, Treatment = 0.5, Death = 1.5),
  y = c(Z1 = 0.9, Z2 = 0.55, Z3 = -0.1, Z4 = 0.2, Treatment = 0.35, Death = 0.35)
)
plot(dag)
```
...
generate_data_inf <- function(n = 500) {
  ...
  logit_p <- (alpha_0 + alpha_Z1 * Z1 + alpha_Z2 * Z2 + alpha_Z3 * Z3 + alpha_Z1sq * (Z1^2) + alpha_Z2Z3 * (Z2 * Z3) + alpha_Z4 * Z4) / 14
  p <- plogis(logit_p) # exp(logit_p) / (1 + exp(logit_p))
  D <- rbinom(n, 1, p) # Treatment indicator
  
  hazard <- exp(beta_0 + beta_D * D + beta_Z1 * Z1 + beta_Z2 * Z2 + beta_Z3 * Z3 + beta_Z4 * Z4 + beta_Z1sq * (Z1^2) + beta_Z2Z3 * (Z2 * Z3) + beta_DZ2 * (D * Z2) + beta_DZ4 * (D * Z4))
  T_event <- rexp(n, rate = hazard)
  
  hazard_cen <- exp(-3.3 + 0.2 * D + 0.4 * Z1 + 0.5 * Z2 + 0.5 * Z3 + 0.4 * Z4 + 0.25 * (Z1^2) + 0.25 * (Z2^2) + 0.25 * (D * Z2) + 0.25 * (D * Z4) + 0.25 * (Z2 * Z3))
...
get_true_survival <- function(data, t, D) {
  hazard <- exp(beta_0 + beta_D * D + beta_Z1 * data$Z1 + beta_Z2 * data$Z2 + beta_Z3 * data$Z3 + beta_Z4 * data$Z4 + 
                beta_Z1sq * (data$Z1^2) + beta_Z2Z3 * (data$Z2 * data$Z3) + beta_DZ2 * (D * data$Z2) + beta_DZ4 * (D * data$Z4))
...
aipwcc_dr_ate_pkg <- function(data, times, ps_correct = TRUE, or_correct = TRUE, 
  ...
  if (ps_correct) {
    ps_model <- glm(D ~ Z1 + Z2 + Z3 + I(Z1^2) + I(Z2 * Z3) + Z4, data = data, family = "binomial")
  } else {
  ...
  if (cen_correct) {
    cen_model <- coxph(Surv(Time, Event == 0) ~ D + Z1 + Z2 + Z3 + Z4 + I(Z1^2) + I(Z2^2) + I(Z2 * Z3) + D:Z2 + D:Z4, data = data, x = TRUE)
  } else {
    cen_model <- coxph(Surv(Time, Event == 0) ~ D + Z1 + Z2 + Z3 + Z4, data = data, x = TRUE)
  }
  
  # Outcome regression (Cox proportional hazard model)
  if (or_correct) {
    or_model <- coxph(Surv(Time, Event) ~ D + Z1 + Z2 + Z3 + Z4 + I(Z1^2) + I(Z2 * Z3) + D:Z2 + D:Z4, data = data, x = TRUE)
  } else {
    or_model <- coxph(Surv(Time, Event) ~ D + Z1 + Z2 + Z3 + Z4, data = data, x = TRUE)
  }
