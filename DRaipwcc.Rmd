...
generate_data_noninf <- function(n = 1000) {
  ...
  Z2 <- rtruncnorm(n, a = -1.5, b = 1.5, mean = 0, sd = 0.5)
  Z3 <- rtruncnorm(n, a = -1, b = 1, mean = 0, sd = 0.3)
  Z4 <- rnorm(n, 0, 1.0)
  Z5 <- rnorm(n, 0, 0.5)
  
  logit_p <- -0.05 + 0.15 * Z1 + 0.30 * Z2 + 0.45 * Z3 + 0.70 * (Z1 * Z2)
  ...
  hazard_cen <- exp(-4.9 + 0.15 * D + 0.08 * Z1 + 0.12 * Z2 + 0.25 * (Z1 * Z2) + 0.25 * Z3 + 0.10 * (Z1^2))
  T_censor <- rexp(n, rate = hazard_cen) # Censoring time based on treatment and other covariates
  T_admin <- rep(12, n) # Administrative (random) censoring time
  ...
  return(data.frame(Z1 = Z1, Z2 = Z2, Z3 = Z3,  Z4 = Z4, Z5 = Z5, D = D, Time = Time, Event = Event, Censored = Censored))
}
```
...
aipwcc_dr_ate_pkg <- function(data, times, ps_correct = TRUE, or_correct = TRUE, 
  ...
  if (ps_correct) {
    ...
    ps_model <- glm(D ~ Z1 + Z4 + Z5, data = data, family = "binomial")
  }

  # Censoring model 
  # Note: Event==0 includes both loss to follow-up and administrative censoring
  if (cen_correct) {
    cen_model <- coxph(Surv(Time, Event == 0) ~ D + Z1 * Z2 + Z3 + I(Z1^2), data = data, x = TRUE)
  } else {
...
kable(bias_table,
      col.names = c("Scenario", "T = 1, 2, 3", "T = 1, 2, 3"),
      caption = "Performance of the AIPWCC estimator (Bias)",
      align = c('c', 'c', 'c')) %>%
  kable_styling(full_width = FALSE, font_size = 12) %>%
  add_header_above(c(" " = 1, "Asymptotic" = 1, "Bootstrap" = 1)) %>%
...
kable(var_table,
      col.names = c("Scenario", "T = 1, 2, 3", "T = 1, 2, 3"),
      caption = "Performance of the AIPWCC estimator (Variance)",
      align = c('c', 'c', 'c')) %>%
  kable_styling(full_width = FALSE, font_size = 12) %>%
  add_header_above(c(" " = 1, "Asymptotic" = 1, "Bootstrap" = 1)) %>%
