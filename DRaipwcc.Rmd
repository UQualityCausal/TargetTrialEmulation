...
generate_data_noninf <- function(n = 500) {
  Z1 <- rtruncnorm(n, a = -2, b = 2, mean = 0, sd = 1)
  Z2 <- rtruncnorm(n, a = -1.5, b = 1.5, mean = 0, sd = 0.7)
  Z3 <- rtruncnorm(n, a = -1, b = 1, mean = 0, sd = 0.55)
  Z4 <- rnorm(n, 0, 1)
  Z5 <- rnorm(n, 0, 1)
  
  logit_p <- (-0.2 + 0.25 * Z1 + 1.2 * Z2 + 1.1 * Z3) / 2.6
  p <- plogis(logit_p) # exp(logit_p) / (1 + exp(logit_p))
  D <- rbinom(n, 1, p) # Treatment indicator
  
  hazard <- exp(beta_0 + beta_D * D + beta_Z1 * Z1 + beta_Z2 * Z2 + beta_Z3 * Z3)
  T_event <- rexp(n, rate = hazard) # Event time from an exponential model
  
  hazard_cen <- exp(-1.7 + 1 * D + 0.15 * Z1 + 0.5 * D * Z1 + 0.6 * Z2 + 0.9 * Z3)
...
get_true_survival <- function(data, t, D) {
  hazard <- exp(beta_0 + beta_D * D + beta_Z1 * data$Z1 + beta_Z2 * data$Z2 + beta_Z3 * data$Z3)
...
aipwcc_dr_ate_pkg <- function(data, times, ps_correct = TRUE, or_correct = TRUE, 
  ...
  if (ps_correct) {
    ...
    ps_model <- glm(D ~ Z4 + Z5 + I(Z4^2) + I(Z5^2), data = data, family = "binomial")
  }

  # Censoring model 
  # Note: Event==0 includes both loss to follow-up and administrative censoring
  if (cen_correct) {
    cen_model <- coxph(Surv(Time, Event == 0) ~ D * Z1 + Z2 + Z3, data = data, x = TRUE)
  } else {
    cen_model <- coxph(Surv(Time, Event == 0) ~ Z4 + Z5 + I(Z4^2) + I(Z5^2), data = data, x = TRUE)
  }
  
  # Outcome regression (Cox proportional hazard model)
  if (or_correct) {
    or_model <- coxph(Surv(Time, Event) ~ D + Z1 + Z2 + Z3, data = data, x = TRUE)
  } else {
    or_model <- coxph(Surv(Time, Event) ~ Z4 + Z5 + I(Z4^2) + I(Z5^2), data = data, x = TRUE)
  }
