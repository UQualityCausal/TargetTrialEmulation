...
-   At the individual level, the potential outcomes $Y(1)$ and $Y(0)$ under two treatment are the outcomes that would be observed if the subject had taken hormone therapy and had never taken the hormone therapy, respectively.
...
-   In comparison, at the population level, the potential mean outcomes $E[Y(1)]$ and $E[Y(0)]$ under two treatment are the average of the outcomes that would be observed if all subjects had taken hormone therapy and had never taken the hormone therapy, respectively.
...
-   Given this, the mean outcome in the hormone therapy group equals to the potential outcome had everyone in the study taken hormone therapy.
    Similarly, the mean outcome in the usual care group equals to the potential outcome had everyone in the study never taken usual care.
...
-   **Conditional Exchangeability**
    ...
    -   Failing to statisfy conditional exchangeability is commonly referred to as *confounding by indication* or *endogeneity*.

## Causal inference methods for restoring conditional exchangeability

Various propensity score methods, including matching, weighting, stratification, regression, doubly robust methods [@VanDerLaan2006, @Wager2018], marginal structural models, are widely used to estimate ATEs and they well explained by @Hernan2020.
...
love.plot(bal, var.order = "unadjusted", abs = TRUE, thresholds = c(m = 0.1),
  ...
  labs(caption = "Abbreviations: anti-HTW meds_0 = antihypertensive medication use at baseline;
       ...
       \nDBP = Standardized diastolic blood pressure at baseline")#check DBP???
```
...
```{r fig.width=5, fig.height=3}
# Log-log plot: Parallel lines indicate proportional hazards assumption is met.???check plot
...
obsdata5 <- obsdata5 %>%
  mutate(
    ...
    censor_weight = if_else(
      follow_up == 0, 1, # Baseline should be 1
      ifelse(C_neg == 1, # Calculate censoring weights (unstablized)
...
filtered_data <- obsdata5 %>% filter(C != 1) # Exclude censored observations (C=1) because their outcomes are unknown
filtered_data$combi_weight <- as.numeric(filtered_data$combi_weight)

weighted_iptw_ipcw_model <- glm(Y ~ assigned_treatment * follow_up + I(follow_up^2), #change back to assigned_treatment + follow_up???
                                data = filtered_data, 
...
```{r}
#revise influence function (remove competing risks) to get SE (not bootstrap)???
```

```{r}
#find ITT truth, add truth column, CI???
```
